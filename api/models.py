...
from .constants import CategoriaEquipo, EstadoOrden, Prioridad
...
class Equipo(models.Model):
    ...
    categoria = models.IntegerField(choices=CategoriaEquipo.choices, verbose_name='Categoría')
...
class Mantenimiento(models.Model):
    ...
    estado = models.IntegerField(choices=EstadoOrden.choices, default=1, verbose_name='Estado')
    tecnico_asignado = models.CharField(max_length=100, blank=True, verbose_name='Técnico')
...
class Recurso(models.Model):
    ...
    tipo = models.CharField(max_length=20, choices=TIPOS, verbose_name='Tipo de recurso')
    nombre = models.CharField(max_length=100, verbose_name='Nombre')
    especialidad = models.CharField(max_length=50, blank=True, verbose_name='Especialidad')
    ...
    def necesita_reposicion(self):
        return self.stock < self.stock_minimo if self.tipo == self.TIPO_REPUESTO else False
...
class DatoEntrenamiento(models.Model):
    ...
    categoria_equipo = models.IntegerField(choices=CategoriaEquipo.choices, null=True, blank=True)
    ...
    conjunto = models.CharField(max_length=20, blank=True)
    fecha_creacion = models.DateTimeField(auto_now_add=True)

    class Meta:
        db_table = 'api_dato_entrenamiento'
...
class ModeloIA(models.Model):
    ...
    datos_entrenamiento = models.IntegerField(default=0, verbose_name='Datos de entrenamiento')
    epocas_completadas = models.IntegerField(default=0, verbose_name='Épocas completadas')
    historial_metricas = models.JSONField(default=list, verbose_name='Historial de métricas')
    fecha_ultimo_entrenamiento = models.DateTimeField(null=True, blank=True)
    modelo_path = models.CharField(max_length=500, blank=True, verbose_name='Path del modelo')
    activo = models.BooleanField(default=True, verbose_name='¿Activo?')
    fecha_creacion = models.DateTimeField(auto_now_add=True)

    class Meta:
        db_table = 'api_modelo_ia'
...
class AprendizajeAutomatico(models.Model):
    ...
    class Meta:
        db_table = 'api_aprendizaje_automatico'
...
class BaseConocimiento(models.Model):
    ...
    class Meta:
        db_table = 'api_base_conocimiento'
...
class Recomendacion(models.Model):
    ...
    ahorro_estimado = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
